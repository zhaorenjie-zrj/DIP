...
def main():
    ...
    # Initialize datasets and dataloaders# 图像和标签的根目录
    train_dir = 'pix2/datasets/facades/train'
    val_dir = 'pix2/datasets/facades/val'
    ...
    if not os.path.exists(train_list_path):
        print(f"Error: {train_list_path} does not exist.")
    if not os.path.exists(val_list_path):
        print(f"Error: {val_list_path} does not exist.")
