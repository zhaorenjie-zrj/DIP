...
def laplacian_filter(image_tensor):
    # 定义拉普拉斯卷积核
    laplacian_kernel = torch.tensor([[0, 1, 0],
                                     [1, -4, 1],
                                     [0, 1, 0]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # shape: (1, 1, 3, 3)
    laplacian_kernel = laplacian_kernel.repeat(3, 1, 1, 1)  # 针对RGB通道扩展卷积核

    # 应用卷积（对每个通道进行拉普拉斯滤波）
    laplacian_image = F.conv2d(image_tensor, laplacian_kernel, padding=1, groups=3)
    return laplacian_image
def translate_fore(mask, dx, dy):
    #将前景平移指定的偏移量。
    return torch.roll(mask, shifts=(dy, dx), dims=(2, 3))


# Calculate the Laplacian loss between the foreground and blended image
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask,dx,dy):
    """
    ...
    # 计算前景和融合图像的拉普拉斯图
    foreground_img=translate_fore(foreground_img, dx, dy)
    laplacian_foreground = laplacian_filter(foreground_img)
    laplacian_blended = laplacian_filter(blended_img)
    
    # 计算拉普拉斯图之间的平方差
    laplacian_diff = (laplacian_foreground - laplacian_blended) ** 2
    
    # 将掩膜扩展到RGB通道，并计算总损失
    mask_expanded = background_mask.repeat(1, 3, 1, 1)
    loss = torch.sum(laplacian_diff * mask_expanded)
...
def close_polygon_and_reset_dx(img_original, polygon_state, dx, dy, background_image_original):
    ...
    return img_with_poly, updated_polygon_state, updated_background, new_dx#有问题
